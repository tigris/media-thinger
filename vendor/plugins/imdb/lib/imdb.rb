...
require 'open-uri'
require 'date'
require 'imdb_company'
require 'imdb_movie'
require 'imdb_name'
require 'imdb_genre'
require 'imdb_plot_keyword'
...
class Imdb
  ...
  def self.find_movie_by_id(id)
    ...
    movie.imdb_id = id
    movie.title = data.at("meta[@name='title']")['content']
    if year = movie.title.match(/\((\d{4})(?:\/\w+)\)/)
      movie.year = year[1]
      movie.title = movie.title.gsub(/\(\d\d\d\d(?:\/\w+)?\)/,'').strip
    end
    ...
    infos.each do |info|
      ...
      when 'Plot Keywords:'
        movie.plot_keywords = parse_plot_keywords(info)
      when "Plot:"
  ...
  def self.parse_plot_keywords(info)
    plot_links = (info/'a').find_all do |link|
      link['href'] =~ /^\/keyword\//
    end
    plot_links.map do |link|
      ImdbPlotKeyword.new(link.inner_html, link['href'])
    end
  end
